...
- 版：v0.2
- 更新日：2026-09-16
- 状態：確認事項への回答を反映した。残りは「11. 要確認事項」で決める
...
| バックアップ | 1日1回以上データベースのバックアップを取り、アプリとは別の場所に保管する（「9. 技術構成（案）」の注意を参照） |
| 通信 | HTTPS のみ |
...
| 役割 | 案 | 理由 |
|---|---|---|
| 画面とサーバー処理 | Next.js（TypeScript） | 画面とサーバー処理を1つのプロジェクトで作れる |
| データベース・ログイン・ファイル保存 | Supabase（PostgreSQL・Auth・Storage） | ログインの仕組みがそろっている。在庫の引当をデータベースのトランザクションで安全に処理できる。将来の温度ログも保存できる |
| 公開（ホスティング） | Vercel | Next.js をそのまま公開できる |

**費用と運用の注意**（2026-09-16 に各サービスの公式ドキュメントと料金ページで確認）

- Supabase の無料プラン
  - 7日間ほとんど使われないと、プロジェクトが一時停止される。毎日少しでも使っていれば、ふつうは止まらない。止まっても 90日以内なら管理画面から再開できる
  - バックアップをダウンロードできない。無料プランでは、`supabase db dump` で定期的にデータを書き出し、別の場所に保管するよう公式が勧めている
  - データベースは 500MB まで（フェーズ1〜2のデータ量なら十分）
  - 有料の Pro プラン（月 $25 から）なら、一時停止されず、毎日自動でバックアップされて直近7日分から戻せる
- Vercel の無料（Hobby）プランは、個人の非商用利用に限られる。商用利用は「制作に関わる誰かの金銭的な利益のために使われるもの」と広く定義されており、お店の業務用アプリは当たる可能性が高い。有料の Pro プラン（月 $20）にするか、商用利用できる別のホスティングを選ぶ（判断に迷う場合は Vercel のサポートに問い合わせるよう案内されている）

費用をかけるかどうかで構成が変わるため、要確認 #12・#13 で決める。開発は自分の PC 上で進められるので、決めるのは本番で使い始める前でよい。
...
| # | 確認したいこと | 仮の案 |
|---|---|---|
| 12 | 公開先：Vercel の有料プラン（月 $20）にするか、商用利用できる別のホスティングにするか。Vercel の無料プランは業務用アプリに使えない可能性が高いため、どちらかを選ぶ必要がある | 未定。本番で使い始める前に決める |
| 13 | データベース：Supabase の有料プラン（月 $25 から）にするか、無料プランのまま使うか。有料にすると、一時停止されず、バックアップが自動になる。無料のままなら、自分で毎日バックアップを取る仕組みを作る | 未定。本番で使い始める前に決める |
